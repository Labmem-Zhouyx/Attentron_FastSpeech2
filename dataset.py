...
class Dataset(Dataset):
    ...
    def reprocess(self, data, idxs):
        ...
        ref_coarse_mels = [data[idx]["ref_coarse_mel"] for idx in idxs]
        ref_fine_mels = [data[idx]["ref_fine_mel"] for idx in idxs]

        text_lens = np.array([text.shape[0] for text in texts])
        mel_lens = np.array([mel.shape[0] for mel in mels])
        ref_coarse_mel_lens = np.array([ref_coarse_mel.shape[0] for ref_coarse_mel in ref_coarse_mels])
        ref_fine_mel_lens = np.array([ref_fine_mel.shape[0] for ref_fine_mel in ref_fine_mels])

        speakers = np.array(speakers)
        texts = pad_1D(texts)
        mels = pad_2D(mels)
        ref_coarse_mels = pad_2D(ref_coarse_mels)
        ref_fine_mels = pad_2D(ref_fine_mels)
...
class TextDataset(Dataset):
    ...
    def collate_fn(self, data):
        ...
        return ids, raw_texts, texts, text_lens, max(text_lens), ref_mels, ref_mel_lens, ref_mels, ref_mel_lens
